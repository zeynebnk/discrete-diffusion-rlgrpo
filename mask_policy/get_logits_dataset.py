...
def generate_dataset():
    ...
    prompts = get_gsm8k_questions("train[:5]")['question']
    print(prompts)
    
    agg_X, agg_y = torch.tensor([]), torch.tensor([])
    ...
    with open("logits_ds_X.pkl", "wb") as f:
        pickle.dump(agg_X, f)
    with open("logits_ds_y.pkl", "wb") as f:
        pickle.dump(agg_y, f)

def create_datapoint(agg_X, agg_y, logits, remasked_idx, timestep):

    agg_X = torch.cat([agg_X, logits])
    agg_y = torch.cat([agg_y, remasked_idx])
